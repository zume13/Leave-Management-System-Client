...
@Component({
  ...
  imports: [Modal, ToastContainer, ReactiveFormsModule, NgClass, DatePipe, FormsModule],
...
export class EmployeeRequests implements OnInit{
  ...
  requests = signal<GetAllRequestsByEmployeeDto[]>([]);
  ...
  changedFilter(filter : string){
    ...
    switch(filter){
      case 'all' : this.setRequests(filter);
        break;
      case 'approved' : this.setRequests(filter);
        break;
      case 'rejected' : this.setRequests(filter);
        break;
      case 'cancelled' : this.setRequests(filter);
        break;
      case 'pending' : this.setRequests(filter);
        break;
    }

  }

  setRequests(filter : string){
    this.requests.set([]);
    ...
    if(filter === 'all'){
      ...
      this.requests.set(this.query.EmployeeDashBoardData()!)
    }

    const requests = this.query.EmployeeDashBoardData()?.filter(values => values.status.toLowerCase() === filter);
  ...
  const { leaveType, startDate, endDate, description } = this.LeaveForm.getRawValue();

  console.log(startDate)

    this.command.createRequest(startDate, endDate, description, id, leaveType).subscribe({
      next : () => {
        this.toast.show('Request created successfully');
        this.getEmployeeRequests();
